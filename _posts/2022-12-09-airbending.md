...
This project leverages said phenomenon, by integrating an array of piezoelectric transducers into a stage that levitates and drives a silicon wafer or “puck” on air.


The greater goal of the project is to deterministically control (closed-loop) the puck/silicon wafer through an arbitrary trajectory.

The expectation is that by sampling the control space of different amplitude and phase shift patterns it would be possible to determine a mapping from amplitude and phase shift patterns to elemental translational and rotational displacements, and velocities too, possibly.

Currently approaching the learning phase, this portfolio post is a high-level cursor over several implementation aspects and decisions, outlining the mechanical design, the electrical schematics and components attempted and/or tested and those selected, the respective layers of the control architecture, and the data collection pipeline.



### Mechanical Design

![Final CAD Design]({{ site.baseurl }}/assets/images/8.jpg)
...
The second and settled approach was to use the existing national instrument lab equipment: in particular, a single 32 channel analog output module controlled/commanded through LabVIEW.
This greatly simplified the PCB design requirements and avoided an otherwise arduous solder job ahead.
...
Vision System was developed to 
...
The experiment_manager.py module applies the data_collection sequence to a series of control_inputs. This then becomes the high-level utility for specifying different experimental ideas. The __ex__constant_detla_scanning.py use the experiment_manager.py module to investigate col****
...
Locates the puck
Begins recording the puck’s location and orientation with time stamps
Actuates the transducer by effecting a control dictionary based on the puck’s location/subgrid
Waits a specified run duration before stopping the transducers
Stops recording the puck’s movement
The finally saves the run’s data to a specified csv file.
...
The __ex__constant_delta_scanning.py script, for example, implements an experiment of maintaining similar phase control to transducers along a column of the subgrid andex a constant phase shift from column to column.
...
Next Steps? Q-learning random sampling!